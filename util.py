...
def get_random_song_slice(data, sequence_length):
    ...
    sliced_song = data[random_start_index: random_start_index+sequence_length]

    assert len(sliced_song) <= sequence_length
...
def show_values(pc, song, fmt="%.2f", **kw):
    ...
    for p, color in zip(pc.get_paths(), pc.get_facecolors()):
        ...
        if y_idx<len(song) and x_idx<len(song[0]):
            ax.text(x, y, repr(song[y_idx][x_idx])[1:-1], fontsize=15, ha="center", va="center", color=color, **kw)
